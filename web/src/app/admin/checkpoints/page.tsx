...
const columns: ColumnDef<Checkpoint>[] = [
  ...
  },
  {
    ...
    header: "NFC Tag",
    cell: ({ row }) => row.original.nfc_tag_id || "Belum dipasang",
  },
  {
    id: "actions",
    header: "Aksi",
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2">
          <EditDialog checkpoint={cp} />
          <form action={deleteCheckpoint.bind(null, cp.id)}>
    ...
    <Dialog>
      ...
      <DialogContent>
        ...
        <form action={updateCheckpoint.bind(null, checkpoint.id)} className="space-y-3">
          <div>
            <Label>Nama</Label>
            <Input name="name" defaultValue={checkpoint.name} required />
          </div>
          <input type="hidden" name="site_id" value={checkpoint.site_id} />
          <div>
            <Label>Latitude</Label>
            <Input name="latitude" type="number" step="any" defaultValue={checkpoint.latitude} required />
          </div>
          <div>
            <Label>Longitude</Label>
            <Input name="longitude" type="number" step="any" defaultValue={checkpoint.longitude} required />
          </div>
          <div>
            <Label>Urutan Tampilan</Label>
            <Input name="display_order" type="number" defaultValue={checkpoint.display_order} />
          </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Tambah Checkpoint Baru</DialogTitle>
            </DialogHeader>
            <form action={createCheckpoint} className="space-y-3">
              ...
              <div>
                <Label>Nama</Label>
                <Input name="name" placeholder="Toilet Lt. 1" required />
              </div>
              <div>
                <Label>Latitude</Label>
