...
export default function CheckpointScanScreen({ onSessionStarted }: {
  ...
  const handleScan = async (identifier: string) => {
    ...
    const res = await startSession(
      mode === "nfc" ? identifier : identifier,
      mode,
      loc.coords.latitude,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.modeRow}>
        ...
        >
          <Text style={[styles.modeText, mode === "qr" && styles.modeActiveText]}>
            QR Code
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {mode === "nfc" && (
        <View style={styles.nfcContainer}>
          <Text style={styles.nfcHint}>
            Input NFC Tag ID manual (simulasi NFC untuk development)
          </Text>
          <TextInput
            style={styles.input}
            placeholder="Masukkan NFC Tag ID"
            value={nfcInput}
      ...
      {/* QR Scanner */}
      {mode === "qr" && scanning && hasPermission && (
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    fontSize: 16, width: "100%", marginBottom: 16, textAlign: "center",
  },
  scanBtn: {
  ...
  qrScanner: { flex: 1 },
  qrHint: { textAlign: "center", color: "#fff", fontSize: 14, padding: 12,
    backgroundColor: "rgba(0,0,0,0.6)", position: "absolute", bottom: 80,
    left: 0, right: 0 },
  cancelBtn: {
    backgroundColor: "#fff", borderRadius: 8, padding: 12, alignItems: "center",
    marginTop: 12,
  },
